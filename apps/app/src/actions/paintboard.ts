...
export const getUserPaintboardTasks = withUserAuth(
  'paintboard/getUserPaintboardTasks',
...
// 批量获取任务状态（用于轮询）
export const getPaintboardTasksStatus = withUserAuth(
  'paintboard/getPaintboardTasksStatus',
  async ({ args, orgId }: AuthWrapperContext<{ taskIds: string[] }>) => {
    const { taskIds } = args;
...
// 获取特定任务
export const getPaintboardTask = withUserAuth('paintboard/getPaintboardTask', async ({ args }: AuthWrapperContext<{ taskId: string }>) => {
  const { taskId } = args;
...
export const submitGenerationTask = withUserAuth(
  'paintboard/submitGenerationTask',
  async ({ args, orgId }: AuthWrapperContext<{ model: string; params: SubmitTaskParams }>) => {
