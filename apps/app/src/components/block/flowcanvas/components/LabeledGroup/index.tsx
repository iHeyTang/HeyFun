...
export const GroupNode = forwardRef<HTMLDivElement, GroupNodeProps>(({ label, position = 'top-left', selected, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        'bg-muted/30 border-muted-foreground/20 border-1 relative h-full w-full rounded-lg border-dashed',
        selected && 'border-primary/50',
      )}
      style={{ width: 'calc(100% + 100px)', height: 'calc(100% + 100px)', margin: '-50px' }}
    >
      <NodeResizer minWidth={100} minHeight={100} lineStyle={{ border: 'none' }} handleStyle={{ border: 'none', backgroundColor: 'transparent' }} />
      {label && <div className="bg-muted/80 text-muted-foreground absolute left-2 top-2 rounded px-2 py-1 text-xs font-medium">{label}</div>}
    </div>
  );
});
