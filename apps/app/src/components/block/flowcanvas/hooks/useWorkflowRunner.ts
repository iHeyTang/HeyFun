...
export function useWorkflowRunner({ onSchemaChange }: { onSchemaChange?: (schema: CanvasSchema) => void }) {
  ...
  const runWorkflow = useCallback(
    async (
      ...
      return UnifiedScheduler.runWorkflow(schema, triggerNodeId, context, { nodeExecutors, onNodeOutputChange: handleNodeOutputChange });
    },
    [context, handleNodeOutputChange],
  );
