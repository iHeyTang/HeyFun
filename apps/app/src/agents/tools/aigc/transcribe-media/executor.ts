...
import AIGC, { speechToTextParamsSchema } from '@/llm/aigc';
...
export const transcribeMediaExecutor = definitionToolExecutor(transcribeMediaParamsSchema, async (args, context) => {
  try {
    ...
    if (!selectedModel) {
      ...
      if (sttModels.length === 0) {
        return {
          success: false,
          error: 'No speech-to-text models available. Please configure a speech recognition provider.',
        };
      }
      ...
      if (!selectedModel) {
        return {
          success: false,
          error: 'Failed to select a speech-to-text model',
        };
      }
    }
    ...
    if (!modelInstance) {
      return {
        success: false,
        error: `Model "${selectedModel}" not found. Use get_aigc_models to see available models.`,
      };
    }
    ...
    while (attempts < maxAttempts) {
      ...
      if (completedTask.status === 'completed') {
        ...
        // 重新获取任务结果（从 AIGC 系统）
        const taskModelInstance = AIGC.getModel(completedTask.model);
        if (!taskModelInstance) {
          return {
            success: false,
    ...
    return {
      success: false,
      error: 'Task timeout: transcription took too long',
    };
  } catch (error) {
    return {
      success: false,
