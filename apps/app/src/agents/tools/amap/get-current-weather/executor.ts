...
import { getAmapApiKey, callAmapApi, geocodeAddress } from '../utils';

/**
 * 高德地图天气API
 * 文档: https://lbs.amap.com/api/webservice/guide/api/weatherinfo
 */
async function getWeatherFromAmap(city: string, context: ToolContext) {
...
export const getCurrentWeatherExecutor = definitionToolExecutor(getCurrentWeatherParamsSchema, async (args, context) => {
  return await context.workflow.run(`toolcall-${context.toolCallId}`, async () => {
    try {
      const { city, unit = 'celsius' } = args;

      const weatherData = await getWeatherFromAmap(city, context);
