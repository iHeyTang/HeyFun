...
当你需要在回复中展示地图位置或路径规划时，可以使用特殊的地图语法：
...
参数说明：
- location（必需）：经纬度坐标，格式为 "经度,纬度"（如 "116.397428,39.90923"）
- name（可选）：地点名称，用于显示在地图上
- zoom（可选）：地图缩放级别，范围 3-18，默认 14
- width（可选）：地图宽度（像素），默认 600
- height（可选）：地图高度（像素），默认 400

示例：
\`<!-- map: {"location": "116.397428,39.90923", "name": "天安门", "zoom": 15} -->\`

### 路径规划模式

语法格式：
\`<!-- map: {"from": {"location": "起点经纬度", "name": "起点名称"}, "to": {"location": "终点经纬度", "name": "终点名称"}, "routeType": "交通方式", "policy": 策略编号, "routeData": {路径数据}} -->\`

参数说明：
- from（必需）：起点信息对象
  - location：起点经纬度，格式为 "经度,纬度"
  - name：起点名称（可选）
- to（必需）：终点信息对象
  - location：终点经纬度，格式为 "经度,纬度"
  - name：终点名称（可选）
- routeType（可选）：交通方式，可选值：
  - "car"（驾车，默认）
  - "walk"（步行）
  - "bike"（骑行）
  - "bus"（公交）
- policy（可选）：路线策略，仅驾车和公交有效
  - 驾车：1=最快（默认）、2=最短、3=避免高速、4=避免拥堵、5=避免收费
  - 公交：1=最快到达、2=最少换乘、3=最少步行、4=最舒适
- routeData（强烈推荐）：MCP direction 工具返回的路径数据对象
  - path：路径点数组，格式为 [[经度, 纬度], ...]
  - distance：距离（米）
  - duration：时长（秒）
  - **重要**：如果提供了 routeData，地图组件将直接使用这些数据绘制路线，否则会实时调用 API
- width（可选）：地图宽度（像素），默认 600
- height（可选）：地图高度（像素），默认 400
...
示例 1（推荐方式 - 带 MCP 路径数据）：
当用户询问路线时，先调用 direction 工具获取路径数据，然后使用地图语法展示：

\`<!-- map: {"from": {"location": "120.093988,30.129412", "name": "西投西悦公寓(云栖小镇店)"}, "to": {"location": "120.081292,30.12859", "name": "杭州龙源壹度空间科技有限公司"}, "routeType": "car", "policy": 1, "routeData": {"path": [[120.093988,30.129412], [120.091234,30.128956], [120.081292,30.12859]], "distance": 3200, "duration": 900}} -->\`

示例 2（简化方式 - 实时规划）：
如果没有调用 direction 工具，也可以只提供起点终点，地图组件会实时规划：

\`<!-- map: {"from": {"location": "120.093988,30.129412", "name": "西投西悦公寓"}, "to": {"location": "120.081292,30.12859", "name": "龙源壹度空间"}, "routeType": "car"} -->\`

使用场景：
- 单点模式：用户询问某个地点的位置时、POI 搜索结果展示
- 路径模式：用户询问"怎么去"、"如何到达"、路线规划需求时

工作流程（路径规划）：
1. 用户询问路线规划问题
2. 调用 direction MCP 工具获取起点、终点坐标和完整路径数据
3. 在文本中先描述路线信息（距离、时间等）
4. 使用地图语法插入可视化地图，**务必传入 routeData 参数**
5. 用户可以在地图上直观看到完整路线

注意事项：
- 必须先通过高德地图工具获取准确的经纬度坐标
- **路径规划时强烈建议传入 routeData**，这样可以避免重复 API 调用，提升响应速度
- 地图语法必须单独成行，前后留空行
- 确保 JSON 格式正确，所有字符串值使用双引号
- 路径规划时 from 和 to 都必须提供
- 可以在一个回复中插入多个地图（单点或路径）
- 路径规划优先使用用户指定的交通方式，未指定时默认使用驾车
`;
}
