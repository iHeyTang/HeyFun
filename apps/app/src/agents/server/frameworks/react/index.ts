...
export abstract class ReactAgent extends BaseAgent {
   ...
   */
  async *stream(llmClient: ChatClient, input: string | UnifiedChat.Message[], history: UnifiedChat.Message[] = []): AsyncGenerator<ReactStreamChunk> {
    // 构建消息历史
    let messages: UnifiedChat.Message[];

    if (typeof input === 'string') {
      // 字符串输入：构建新的消息历史
      messages = [{ role: 'system', content: this.config.systemPrompt }, ...history, { role: 'user', content: input }];
    } else {
      // 消息数组输入：合并系统提示词和历史
