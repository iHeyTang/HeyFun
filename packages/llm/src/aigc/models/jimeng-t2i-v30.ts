 ...
 */
export class JimengT2iV30 extends BaseAigcModel {
  name = 'jimeng-t2i-v30';
  displayName = '即梦 3.0(文生图)';
  ...
  async getTaskResult(params: { generationType: string; model: string; taskId: string }): Promise<GenerationTaskResult> {
    const parsed = jimengT2iV30GetResultParamsSchema.safeParse({
    ...
    const result = await this.provider.t2iV30GetResult(parsed.data);
    return {
